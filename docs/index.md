...
<a href="https://github.com/hustzxd/EfficientPaper" target="_blank" class="github-corner" title="View on GitHub">
...
<div id="paper-search-app">
  ...
  <!-- Edit Paper Modal -->
  <div id="edit-paper-modal" class="arxiv-modal edit-paper-modal" style="display: none;">
    <div class="arxiv-modal-content">
      <div class="arxiv-modal-header">
        <h2 id="edit-paper-modal-title">Edit Paper</h2>
        <button id="close-edit-paper" class="close-arxiv" title="Close editor">✕</button>
      </div>
      <div class="arxiv-modal-body">
        <iframe id="edit-paper-frame" title="Edit paper metadata"></iframe>
      </div>
    </div>
  </div>
...
/* Embedded paper editor */
.edit-paper-modal .arxiv-modal-content {
...
.edit-paper-modal .arxiv-modal-header {
  flex: 0 0 auto;
  padding: 14px 20px;
}

.edit-paper-modal .arxiv-modal-body {
  flex: 1 1 auto;
  min-height: 0;
  padding: 0;
  overflow: hidden;
...
.paper-card:hover .note-indicator {
  opacity: 1;
}
...
(function() {
  ...
  const errorIconSvg = '<svg viewBox="0 0 24 24"><path d="M19 6.41 17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg>';

  // Expose variables to window for auto-reload state restoration
  window.papers = papers;
  window.filteredPapers = filteredPapers;
  window.currentPage = currentPage;
  let isLocalServerAvailable = false; // Track local server availability
  ...
  function updateServerDependentUI() {
    ...
    document.querySelectorAll('.edit-link').forEach(link => {
      ...
      if (!isLocalServerAvailable) {
        if (link.href) link.dataset.href = link.href;
  ...
  function getPaperShareId(paper) {
    return paper.prototxt_path || paper.id || paper.abbr || paper.title;
  }

  function getPaperShareUrl(paper) {
    return paper.url || paper.note_url || window.location.href;
  }

  function buildPaperShareText(paper, link) {
    const authorText = Array.isArray(paper.authors) && paper.authors.length
    ...
    return [
      ...
      link || getPaperShareUrl(paper)
    ].filter(Boolean).join('\n');
  }

  function buildPaperShareLink(paper) {
    const url = new URL(window.location.href);
    url.hash = '';
    url.search = '';
    url.searchParams.set('paper', getPaperShareId(paper));
    return url.toString();
  }
  ...
  function clearPaperShareParam() {
    activeSharedPaperId = null;
    const url = new URL(window.location.href);
    if (!url.searchParams.has('paper')) return;
    url.searchParams.delete('paper');
    history.replaceState({}, '', url);
  }

  function findPaperByShareId(shareId) {
    ...
    return papers.find(paper => {
      ...
      return candidates.includes(String(shareId));
    }) || null;
  }

  function focusSharedPaperCard(shareId) {
    requestAnimationFrame(() => {
      const cards = Array.from(document.querySelectorAll('.paper-card'));
      const targetCard = cards.find(card => card.dataset.shareId === String(shareId));
      if (!targetCard) return;
      targetCard.scrollIntoView({ behavior: 'smooth', block: 'center' });
    });
  }

  function showSharedPaper(paper, options = {}) {
    const shareId = getPaperShareId(paper);
    activeSharedPaperId = shareId;
    ...
    document.getElementById('sort-filter').value = 'update-desc';
    ...
    if (options.updateUrl !== false) {
      ...
      url.hash = '';
      url.search = '';
      url.searchParams.set('paper', shareId);
      history.replaceState({}, '', url);
    }

    renderPapers(filteredPapers);
    updateStats();
    focusSharedPaperCard(shareId);
  }

  function applySharedPaperFromUrl() {
    const urlParams = new URLSearchParams(window.location.search);
    const paperParam = urlParams.get('paper');
    if (!paperParam) return false;

    const paper = findPaperByShareId(paperParam);
    if (!paper) {
      clearPaperShareParam();
      return false;
    }

    showSharedPaper(paper, { updateUrl: false });
    return true;
  }

  function showTemporaryButtonFeedback(button, html, statusClass, originalHTML, duration = 2000) {
    if (!button) return;

    button.disabled = false;
    button.classList.remove('shared', 'failed');
  ...
  window.sharePaperCard = function(paperId, button) {
    const paper = paperShareLookup.get(paperId);
    if (!paper) {
      console.warn('Paper not found for sharing:', paperId);
      return;
    }
    ...
    if (button) {
      button.disabled = true;
      button.innerHTML = `${shareIconSvg} Copying`;
    }

    const shareLink = buildPaperShareLink(paper);
    copyText(shareLink).then(() => {
      showTemporaryButtonFeedback(button, `${successIconSvg} Link Copied`, 'shared', originalHTML, 2400);
    }).catch(error => {
      console.error('Failed to share paper card:', error);
      const fallbackText = buildPaperShareText(paper, shareLink);
      alert(`Copy failed. You can share this link manually:\n\n${fallbackText}`);
      showTemporaryButtonFeedback(button, `${errorIconSvg} Failed`, 'failed', originalHTML);
    });
  };
  ...
  async function loadPapers() {
    try {
      ...
      filteredPapers = [...papers]; // Create a copy
      window.filteredPapers = filteredPapers; // Expose for auto-reload
      const defaultSort = document.getElementById('sort-filter')?.value || 'update-desc';
      sortPapers(filteredPapers, defaultSort);
      renderPapers(filteredPapers);
      updateStats();

      if (applySharedPaperFromUrl()) {
        return;
      }
  ...
  function sortPapers(papersArray, sortType) {
    switch(sortType) {
      case 'update-desc': // Update time (newest first) - default
        papersArray.sort((a, b) => (b.update_time || 0) - (a.update_time || 0));
        break;
      case 'update-asc': // Update time (oldest first)
        papersArray.sort((a, b) => (a.update_time || 0) - (b.update_time || 0));
        break;
      case 'year-desc': // Published year (newest first)
        papersArray.sort((a, b) => {
          if (b.year !== a.year) return b.year - a.year;
          // If same year, sort by abbr alphabetically (case-insensitive)
          const abbrA = (a.abbr || '').toLowerCase();
          const abbrB = (b.abbr || '').toLowerCase();
          return abbrA.localeCompare(abbrB);
        });
        break;
      case 'year-asc': // Published year (oldest first)
        papersArray.sort((a, b) => {
      ...
      case 'rating-desc': // Rating (highest first)
        ...
        break;
      default:
        // Default to update time descending
        papersArray.sort((a, b) => (b.update_time || 0) - (a.update_time || 0));
    }
  }

  // Parse search query with support for AND, OR, NOT operators
  // Syntax:
  //   key1 key2       -> OR (match key1 OR key2)
  //   key1 AND key2   -> AND (match both)
  //   key1 & key2     -> AND (match both)
  //   key1 | key2     -> OR (match either)
  //   key1 -key2      -> NOT (match key1 but NOT key2)
  //   key1 NOT key2   -> NOT (match key1 but NOT key2)
  ...
  function parseSearchQuery(query) {
    ...
    while ((match = regex.exec(query)) !== null) {
      if (match[1]) {
        ...
        tokens.push({ type: 'exact', value: match[1].toLowerCase() });
      } else if (match[2]) {
        ...
        if (token === 'and' || token === '&') {
          ...
          tokens.push({ type: 'not', value: token.slice(1) });
        } else {
    ...
    // Convert to search groups (AND groups combined with OR)
    const groups = [];
    let currentGroup = { terms: [], notTerms: [], exact: [] };

    for (let i = 0; i < tokens.length; i++) {
      const token = tokens[i];

      if (token.type === 'or') {
        if (currentGroup.terms.length > 0 || currentGroup.exact.length > 0 || currentGroup.notTerms.length > 0) {
          groups.push(currentGroup);
        }
        currentGroup = { terms: [], notTerms: [], exact: [] };
      } else if (token.type === 'not') {
        // Next token is the term to exclude
        const nextToken = tokens[i + 1];
        if (nextToken && (nextToken.type === 'term' || nextToken.type === 'exact')) {
          currentGroup.notTerms.push(nextToken.value);
          i++; // Skip next token
        }
      } else if (token.type === 'term') {
        currentGroup.terms.push(token.value);
      } else if (token.type === 'exact') {
        currentGroup.exact.push(token.value);
      }
      // 'and' is implicit - terms in same group are AND-ed
    }
  ...
  // Test if paper matches search criteria
  function matchesSearch(paper, searchGroups) {
    const searchFields = [
      paper.title,
      paper.abbr,
      ...paper.authors,
    ...
    // Each group is OR-ed with others
    return searchGroups.some(group => {
      // Within a group, all terms are AND-ed
      let matchesGroup = true;

      // All regular terms must match
      if (group.terms.length > 0) {
        matchesGroup = group.terms.every(term => searchFields.includes(term));
      }

      // All exact phrases must match
      if (matchesGroup && group.exact.length > 0) {
        matchesGroup = group.exact.every(phrase => searchFields.includes(phrase));
      }

      // All NOT terms must NOT match
      if (matchesGroup && group.notTerms.length > 0) {
        matchesGroup = group.notTerms.every(notTerm => !searchFields.includes(notTerm));
      }
  ...
  function filterPapers() {
    if (activeSharedPaperId) {
      clearPaperShareParam();
    }

    const searchQuery = document.getElementById('search-input').value.trim();
    const searchGroups = parseSearchQuery(searchQuery);
    ...
    filteredPapers = papers.filter(paper => {
      // Search query
      if (searchGroups) {
        if (!matchesSearch(paper, searchGroups)) {
          return false;
        }
      }
    ...
    // Apply sorting
    sortPapers(filteredPapers, sortFilter);

    // Expose filteredPapers for auto-reload
    window.filteredPapers = filteredPapers;
    ...
    currentPage = 1;
    window.currentPage = currentPage; // Sync for auto-reload
    renderPapers(filteredPapers);
    updateStats();
  }
  ...
  function renderPapers(papersToRender) {
    ...
    const html = papersToShow.map((paper, index) => {
      ...
      paperShareLookup.set(paperId, paper);
      const isSharedFocus = activeSharedPaperId === shareId ? ' shared-focus' : '';
      ...
      // 封面图片 (使用 basePath 确保路径正确)
      const coverSrc = paper.cover ? `${basePath}/${paper.cover}` : '';
      const coverHtml = paper.cover
        ? `<img src="${coverSrc}" alt="cover" loading="lazy" onclick="openLightbox(event, this.src)">`
        ...
        <div class="paper-card${isSharedFocus}" data-share-id="${escapeHtmlAttr(shareId)}">
          ...
          <div class="paper-content">
            ...
            <div class="paper-links">
              ...
              <button class="share-btn" onclick="sharePaperCard('${paperId}', this)" title="Copy shareable link">
                ${shareIconSvg}
                Share Link
              </button>
              ${codeLink}
              ...
              ${paper.prototxt_path ? `<a href="../edit.html?path=${encodeURIComponent(paper.prototxt_path)}" data-path="${escapeHtmlAttr(paper.prototxt_path)}" title="Requires local server" class="edit-link">Edit 🔧</a>` : ''}
    ...
    // Update server-dependent UI elements (PDF, Edit buttons)
    updateServerDependentUI();
  }
  ...
  // Expose renderPapers for auto-reload state restoration
  ...
  // Expose goToPage for auto-reload state restoration
  ...
  function openEditPaperModal(prototxtPath, paperTitle = '') {
    if (!prototxtPath || !isLocalServerAvailable) return;

    const modal = document.getElementById('edit-paper-modal');
    const frame = document.getElementById('edit-paper-frame');
    const titleEl = document.getElementById('edit-paper-modal-title');

    titleEl.textContent = paperTitle ? `Edit: ${paperTitle}` : 'Edit Paper';
    frame.src = `${basePath}/edit.html?path=${encodeURIComponent(prototxtPath)}&embedded=1`;
    modal.style.display = 'flex';
    document.body.style.overflow = 'hidden';
  ...
  function refreshPapersAfterEdit() {
    const filterState = {
    ...
    loadPapers().then(() => {
      document.getElementById('search-input').value = filterState.search;
      document.getElementById('year-filter').value = filterState.year;
      document.getElementById('venue-filter').value = filterState.venue;
      document.getElementById('keyword-filter').value = filterState.keyword;
      document.getElementById('rating-filter').value = filterState.rating;
      document.getElementById('sort-filter').value = filterState.sort;
      filterPapers();

      if (filterState.page > 1) {
        const totalPages = Math.max(1, Math.ceil(filteredPapers.length / itemsPerPage));
        goToPage(Math.min(filterState.page, totalPages));
      }
    }).catch(error => {
      console.error('Failed to refresh papers after editing:', error);
    });
  }
  ...
  function handleEditorMessage(event) {
    ...
    if (!frame || event.source !== frame.contentWindow || !message ||
        message.source !== 'efficient-paper-editor') {
      return;
    }

    if (message.type === 'paper-saved') {
      closeEditPaperModal();
      // Give the local watcher time to regenerate papers.json before reloading it.
      setTimeout(refreshPapersAfterEdit, 1800);
    } else if (message.type === 'paper-deleted') {
      if (message.path) {
        removeDeletedPaperFromState(message.path);
      }
      closeEditPaperModal();
    } else if (message.type === 'paper-cancelled') {
      closeEditPaperModal();
    }
  }
  ...
  function init() {
    ...
    document.getElementById('paper-list').addEventListener('click', (e) => {
      ...
      if (!editLink || editLink.classList.contains('server-disabled')) return;

      e.preventDefault();
      e.stopPropagation();
      const paper = papers.find(item => item.prototxt_path === editLink.dataset.path);
      openEditPaperModal(editLink.dataset.path, paper?.title || '');
    });

    // Embedded editor modal
    document.getElementById('close-edit-paper').addEventListener('click', closeEditPaperModal);
    document.getElementById('edit-paper-modal').addEventListener('click', (e) => {
      if (e.target.id === 'edit-paper-modal') {
        closeEditPaperModal();
      }
    });
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'Escape') {
        ...
        // Close embedded paper editor modal if open
        const editPaperModal = document.getElementById('edit-paper-modal');
        if (editPaperModal.style.display === 'flex') {
          closeEditPaperModal();
          return;
        }
