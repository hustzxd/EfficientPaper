...
一个用于管理和编辑论文元数据的可视化工具，支持从 PDF 自动提取信息、基于小米 MiMo 大模型自动总结和分类论文、在线编辑和部署到 GitHub Pages。

![Efficient Paper](docs/images/efficient_paper.png)

## 前置配置：小米 MiMo API Key

添加论文时，系统会调用小米 MiMo 大模型（`mimo-v2-flash`）自动生成论文中文总结和关键词分类。使用前需要将 API Key 配置到环境变量中：

```bash
export MIMO_API_KEY="your-mimo-api-key"
```

建议将上述命令添加到 `~/.bashrc` 或 `~/.zshrc` 中以持久化配置。

> MiMo API 地址：`https://api.xiaomimimo.com/v1`，使用 OpenAI 兼容接口。
> 如未配置 API Key，添加论文时会跳过自动总结和分类步骤，不影响其他功能。
...
在搜索页面中，使用 `add_paper_info.sh` 脚本从 PDF 文件自动提取论文信息：

```bash
./add_paper_info.sh ~/Downloads/2512.01278v1.pdf
```

<img src="docs/images/add_from_arxiv.png" width="400">

这个脚本会：
- 自动从 PDF 提取论文标题、作者、摘要等信息
- 调用 MiMo 自动生成论文中文总结（写入 `note.md`）
- 调用 MiMo 自动选择 1-2 个关键词分类
- 自动提取代码仓库链接（如 GitHub）
...
<img src="docs/images/edit1.png" width="400">
...
<img src="docs/images/edit2.png" width="400">
...
确认编辑无误后，在网页中点击上传按钮，输入 commit message 即可一键部署到 GitHub：

<img src="docs/images/upload_to_github.png" width="400">
...
- **AI 能力**: 小米 MiMo (`mimo-v2-flash`)，通过 OpenAI 兼容接口调用，用于论文总结和关键词分类
